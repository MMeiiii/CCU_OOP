...
void SDN_controller::recv_handler (packet *p){
    ...
    int node_num = node::getNodeNum()-1;
...
int main()
{
    ...
    unsigned int node_num, link_num, Updtime, Instime,  SimDuration;
    ...
    //new
    for(int i = 0; i < dst_num; i ++){
        BFS(dst_ID[i], i, dis_old, 0);
    }
    
    //new
    for(int i = 0; i < dst_num; i ++){
        BFS(dst_ID[i], i, dis_new, 1);
    }
     ...
     for(int i = 0; i < dst_num; i ++){
        for(int j = 0; j < node_num; j ++){
            if(dst_ID[i] != j){
                ...
                ctrl_packet_event(con_id, j, dst_ID[i], itr->second, Instime, "update");
            }
    ...
    //ctrl_packet updatetime
    for(int i = 0; i < dst_num; i ++){
        for(int j = 0; j < node_num; j ++){
            cout << "j = " << dis_new[i][j] << "\n";
            if(dst_ID[i] != j){
                auto itr = ans_new_table[j].find(dst_ID[i]);
                ctrl_packet_event(con_id, j, dst_ID[i], itr->second, Updtime, "newupdate");
            }
